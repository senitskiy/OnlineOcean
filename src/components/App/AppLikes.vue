<template>
  <button class="likes btn-clear"
  @click.prevent='toggleFavourite'
  :class='[viewLikesStyle, activeLikesStyle]'
  >
...
export default {
  ...
  mounted () {
    // Убрать с mounted, а добавить в заглушку после успешной загрузки
    this.loadLike(this.likes.status)
  },
  methods: {
    loadLike(status){
      this.$emit('loadLike', status)
    },
    toggleFavourite() {
      this.$emit('toggledLike')
    }
  },
  computed: {
    viewLikesStyle() {
      return 'likes--' + this.view
    },
    activeLikesStyle(){
      if (this.liked === true){
        return 'likes--active'
